...
import { BCVPUCore } from '../../src/core/BCVPUCore';
import { ModuleType, TaskStatus } from '../../src/interfaces/IModule';
import { BCVPUConfig } from '../../src/interfaces/IBCVPUCore';
...
async function main(): Promise<void> {
    ...
    try {
        ...
        const tasks = [
            // Memory tasks
            createTask('mem-001', ModuleType.MEMORY, 'Store user preferences', { userId: 123, preferences: { theme: 'dark' } }),
            createTask('mem-002', ModuleType.MEMORY, 'Cache computation results', { computation: 'fibonacci', result: [1, 1, 2, 3, 5, 8] }),
            createTask('mem-003', ModuleType.MEMORY, 'Retrieve historical data', { timeRange: '2024-01-01:2024-12-31' }),
            
            // Decision-making tasks
            createTask('dec-001', ModuleType.DECISION_MAKING, 'Evaluate investment options', { 
                options: ['stocks', 'bonds', 'crypto'], 
                riskTolerance: 0.7,
                timeHorizon: '5 years'
            }),
            createTask('dec-002', ModuleType.DECISION_MAKING, 'Risk assessment for new project', {
                project: 'AI Implementation',
                budget: 100000,
                timeline: 6
            }),
            createTask('dec-003', ModuleType.DECISION_MAKING, 'Optimize resource allocation', {
                resources: ['CPU', 'Memory', 'Storage'],
                constraints: { budget: 50000, performance: 'high' }
            }),
            
            // Sensory processing tasks
            createTask('sen-001', ModuleType.SENSORY_PROCESSING, 'Analyze camera feed', {
                source: 'camera_1',
                resolution: '1080p',
                fps: 30
            }),
            createTask('sen-002', ModuleType.SENSORY_PROCESSING, 'Process audio signals', {
                channels: 2,
                sampleRate: 44100,
                format: 'WAV'
            }),
            createTask('sen-003', ModuleType.SENSORY_PROCESSING, 'Integrate sensor data', {
        ...
        if (diagnostics.details.hub.modules) {
            diagnostics.details.hub.modules.forEach((moduleInfo: any) => {
                console.log(`\n${moduleInfo.name}:`);
                ...
                switch (moduleInfo.type) {
                    case ModuleType.MEMORY:
                        ...
                        break;
                    case ModuleType.DECISION_MAKING:
                        console.log('AI inference, risk analysis, and decision logic');
                        break;
                    case ModuleType.SENSORY_PROCESSING:
...
function setupEventListeners(core: BCVPUCore): void {
    // System events
    core.on('initialized', () => {
...
async function demonstrateAdvancedFeatures(core: BCVPUCore): Promise<void> {
    ...
    const priorityTasks = [
        createTask('priority-001', ModuleType.DECISION_MAKING, 'Emergency decision required', { priority: 'high' }),
        createTask('priority-002', ModuleType.SENSORY_PROCESSING, 'Critical sensor analysis', { priority: 'urgent' })
    ];
